...
class Stage5d(nn.Module):
    ...
    def __init__(self):
        ...
        self.stage4to5d = REBNCONV(512, 256, dirate=1)

        # 分辨率：32 -> 32
        self.stage5to5d = REBNCONV(512, 256, dirate=1)

        # 分辨率：16 -> 32
        self.up6to5d = nn.Upsample(scale_factor=2, mode='bilinear')
        self.stage6to5d = REBNCONV(512, 512, dirate=1)
    ...
    def forward(self, hx1, hx2, hx3, hx4, hx5, hx6):
        ...
        hx5d = self.stage5d(torch.cat((hx4to5d, hx5to5d, hx6to5d), 1))
        ##
...
class Stage4d(nn.Module):
    ...
    def __init__(self):
        ...
        self.stage3to4d = REBNCONV(256, 256, dirate=1)

        # 分辨率：64 -> 64
        self.stage4to4d = REBNCONV(512, 256, dirate=1)

        # 分辨率：16 -> 64
        self.up6to4d = nn.Upsample(scale_factor=4, mode='bilinear')
        self.stage6to4d = REBNCONV(512, 256, dirate=1)

        # 分辨率：32 -> 64
        self.up5dto4d = nn.Upsample(scale_factor=2, mode='bilinear')
        self.stage5dto4d = REBNCONV(512, 256, dirate=1)
    ...
    def forward(self, hx1, hx2, hx3, hx4, hx6, hx5d):
        ...
        hx4d = self.stage4d(torch.cat((hx3to4d, hx4to4d, hx6to4d, hx5dto4d), 1))
        ##
...
class Stage3d(nn.Module):
    ...
    def __init__(self):
        ...
        self.stage2to3d = REBNCONV(128, 128, dirate=1)

        # 分辨率：128 -> 128
        self.stage3to3d = REBNCONV(256, 128, dirate=1)
        ...
        self.stage5dto3d = REBNCONV(512, 128, dirate=1)

        # 分辨率：64 -> 128
        self.up4dto3d = nn.Upsample(scale_factor=2, mode='bilinear')
        self.stage4dto3d = REBNCONV(256, 128, dirate=1)
    ...
    def forward(self, hx1, hx2, hx3, hx6, hx5d, hx4d):
        ...
        hx3d = self.stage3d(torch.cat((hx2to3d, hx3to3d, hx5dto3d, hx4dto3d), 1))
        ##
...
class Stage2d(nn.Module):
    ...
    def __init__(self):
        ...
        self.stage4dto2d = REBNCONV(256, 64, dirate=1)

        # 分辨率：128 -> 256
        self.up3dto2d = nn.Upsample(scale_factor=2, mode='bilinear')
        self.stage3dto2d = REBNCONV(128, 64, dirate=1)
    ...
    def forward(self, hx1, hx2, hx6, hx5d, hx4d, hx3d):
        ...
        hx2d = self.stage2d(torch.cat((hx1to2d, hx2to2d, hx4dto2d, hx3dto2d), 1))
        ##
...
class Stage1d(nn.Module):
    ...
    def __init__(self):
        ...
        ## stage1d
        # 分辨率：512 -> 512
        self.stage1to1d = REBNCONV(64, 32, dirate=1)
        ...
        self.stage3dto1d = REBNCONV(128, 32, dirate=1)

        # 分辨率：256 -> 512
        self.up2dto1d = nn.Upsample(scale_factor=2, mode='bilinear')
        self.stage2dto1d = REBNCONV(64, 64, dirate=1)
    ...
    def forward(self, hx1, hx6, hx5d, hx4d, hx3d, hx2d):
        ...
        hx1d = self.stage1d(torch.cat((hx1to1d, hx3dto1d, hx2dto1d), 1))
        ##
...
class U3U515BiasLayer(nn.Module):

    def __init__(self, in_ch=3, out_ch=1):
        super(U3U515BiasLayer, self).__init__()
    ...
    def forward(self, x):
        ...
        # hx2d = self.stage2d(torch.cat((hx1to2d, hx2to2d, hx6to2d, hx5dto2d, hx4dto2d, hx3dto2d), 1))
        # ##
        hx2d = self.stage2dfunc(hx1, hx2, hx6, hx5d,
                                hx4d, hx3d)
        ...
        # hx1d = self.stage1d(torch.cat((hx1to1d, hx6to1d, hx5dto1d, hx4dto1d, hx3dto1d, hx2dto1d), 1))
        # ##
        # hx1d = self.stage1dfunc(hx1, hx6, hx5d, hx4d,
        #                         hx3d, hx2d)
        hx1d = self.stage1dfunc(hx1, hx6, hx5d, hx4d,
                                hx3d, hx2d)
        # side output
        ...
        d0 = self.outconv(torch.cat((d1,d2,d3,d4,d5,d6),1))
        # print("end")
        return [F.sigmoid(d0), F.sigmoid(d1), F.sigmoid(d2), F.sigmoid(d3), F.sigmoid(d4), F.sigmoid(d5), F.sigmoid(d6)], [hx1d, hx2d,
                                                                                                            hx3d, hx4d,
                                                                                                            hx5d, hx6]
        # return [F.sigmoid(d0)], []
