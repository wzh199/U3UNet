...
class RSU7(nn.Module):
    ...
    def forward(self, x):
        ...
        hx7 = self.rebnconv7(hx4)

        # hx6d = self.rebnconv6d(torch.cat((hx7, hx6), 1))
        # hx6dup = _upsample_like(hx6d, hx5)

        # hx5d = self.rebnconv5d(torch.cat((hx7, hx5), 1))
        # hx5dup = _upsample_like(hx5d, hx4)

        hx4d = self.rebnconv4d(torch.cat((hx7, hx4), 1))
...
class RSU6(nn.Module):
    ...
    def forward(self, x):
        ...
        hx6 = self.rebnconv6(hx4)

        # hx5d = self.rebnconv5d(torch.cat((hx6, hx5), 1))
        # hx5dup = _upsample_like(hx5d, hx4)

        hx4d = self.rebnconv4d(torch.cat((hx6, hx4), 1))
        hx4dup = _upsample_like(hx4d, hx3)
...
class RSU5(nn.Module):
    ...
    def forward(self, x):
        ...
        hx5 = self.rebnconv5(hx3)

        # hx4d = self.rebnconv4d(torch.cat((hx5, hx4), 1))
        # hx4dup = _upsample_like(hx4d, hx3)

        hx3d = self.rebnconv3d(torch.cat((hx5, hx3), 1))
        hx3dup = _upsample_like(hx3d, hx2)
...
class RSU4(nn.Module):
    ...
    def forward(self, x):
        ...
        hx4 = self.rebnconv4(hx2)

        # hx3d = self.rebnconv3d(torch.cat((hx4, hx3), 1))
        # hx3dup = _upsample_like(hx3d, hx2)

        hx2d = self.rebnconv2d(torch.cat((hx4, hx2), 1))
        hx2dup = _upsample_like(hx2d, hx1)
...
#         self.stage1to5d = REBNCONV1(64, 64, dirate=1)
#
#         # 分辨率：256 -> 32
#         self.pool2to5d = nn.MaxPool2d(8, 8, ceil_mode=True)
#         self.stage2to5d = REBNCONV1(128, 64, dirate=1)
#
#         # 分辨率：128 -> 32
#         self.pool3to5d = nn.MaxPool2d(4, 4, ceil_mode=True)
#         self.stage3to5d = REBNCONV1(256, 64, dirate=1)
#
#         # 分辨率：64 -> 32
#         self.pool4to5d = nn.MaxPool2d(2, 2, ceil_mode=True)
#         self.stage4to5d = REBNCONV1(512, 64, dirate=1)
#
#         # 分辨率：32 -> 32
#         self.stage5to5d = REBNCONV1(512, 64, dirate=1)
#
#         # 分辨率：16 -> 32
#         self.up6to5d = nn.Upsample(scale_factor=2, mode='bilinear')
#         self.stage6to5d = REBNCONV1(512, 64, dirate=1)
#
#         # concat -> rsu
...
class Stage4d(nn.Module):
    ...
    def __init__(self):
        ...
        # 分辨率：32 -> 64
        self.up6to4d = nn.Upsample(scale_factor=2, mode='bilinear')
        ...
        # concat -> rsu
        self.stage4d = RSU4(320, 160, 320)
        ##

    def forward(self, hx1, hx2, hx3, hx4, hx6):
        ## stage4d
        # 分辨率：512 -> 64
        ...
        hx4d = self.stage4d(torch.cat((hx1to4d, hx2to4d, hx3to4d, hx4to4d, hx6to4d), 1))
        ##
...
class Stage3d(nn.Module):
    ...
    def __init__(self):
        ...
        # 分辨率：16 -> 128
        self.up6to3d = nn.Upsample(scale_factor=4, mode='bilinear')
        ...
        self.stage4dto3d = REBNCONV(320, 64, dirate=1)

        # concat -> rsu
        self.stage3d = RSU5(320, 160, 320)
        ##

    def forward(self, hx1, hx2, hx3, hx6, hx4d):
        ## stage3d
        # 分辨率：512 -> 128
        ...
        hx3d = self.stage3d(torch.cat((hx1to3d, hx2to3d, hx3to3d, hx6to3d, hx4dto3d), 1))
        ##
...
class Stage2d(nn.Module):
    ...
    def __init__(self):
        ...
        # 分辨率：32 -> 256
        self.up6to2d = nn.Upsample(scale_factor=8, mode='bilinear')
        ...
        self.stage4dto2d = REBNCONV(320, 64, dirate=1)

        # 分辨率：128 -> 256
        self.up3dto2d = nn.Upsample(scale_factor=2, mode='bilinear')
        self.stage3dto2d = REBNCONV(320, 64, dirate=1)

        # concat -> rsu
        self.stage2d = RSU6(320, 160, 320)
        ##

    def forward(self, hx1, hx2, hx6, hx4d, hx3d):
        ## stage2d
        # 分辨率：512 -> 256
        ...
        hx2d = self.stage2d(torch.cat((hx1to2d, hx2to2d, hx6to2d, hx4dto2d, hx3dto2d), 1))
        ##
...
class Stage1d(nn.Module):
    ...
    def __init__(self):
        ...
        # 分辨率：32 -> 512
        self.up6to1d = nn.Upsample(scale_factor=16, mode='bilinear')
        ...
        self.stage4dto1d = REBNCONV(320, 64, dirate=1)

        # 分辨率：128 -> 512
        self.up3dto1d = nn.Upsample(scale_factor=4, mode='bilinear')
        self.stage3dto1d = REBNCONV(320, 64, dirate=1)

        # 分辨率：256 -> 512
        self.up2dto1d = nn.Upsample(scale_factor=2, mode='bilinear')
        self.stage2dto1d = REBNCONV(320, 64, dirate=1)

        # concat -> rsu
        self.stage1d = RSU7(320, 160, 320)
        ##

    def forward(self, hx1, hx6, hx4d, hx3d, hx2d):
        ## stage1d
        # 分辨率：512 -> 512
        ...
        hx1d = self.stage1d(torch.cat((hx1to1d, hx6to1d, hx4dto1d, hx3dto1d, hx2dto1d), 1))
        ##
...
class U3U414(nn.Module):

    def __init__(self, in_ch=3, out_ch=1):
        super(U3U414, self).__init__()
        ...
        # self.stage5dfunc = Stage5d()
        self.stage4dfunc = Stage4d()
        self.stage3dfunc = Stage3d()
        self.stage2dfunc = Stage2d()
        self.stage1dfunc = Stage1d()

        self.side1 = nn.Conv2d(320, out_ch, 3, padding=1)
        self.side2 = nn.Conv2d(320, out_ch, 3, padding=1)
        self.side3 = nn.Conv2d(320, out_ch, 3, padding=1)
        self.side4 = nn.Conv2d(320, out_ch, 3, padding=1)
    ...
    def forward(self, x):
        ...
        # hx5d = self.stage5d(torch.cat((hx1to5d, hx2to5d, hx3to5d, hx4to5d, hx5to5d, hx6to5d), 1))
        # ##
        # hx5d = self.stage5dfunc(hx1, hx2, hx3, hx4,
        #                         hx5, hx6)
        ...
        # hx4d = self.stage4d(torch.cat((hx1to4d, hx2to4d, hx3to4d, hx4to4d, hx6to4d, hx5dto4d), 1))
        # ##
        hx4d = self.stage4dfunc(hx1, hx2, hx3, hx4,
                                hx6)
        ...
        # hx3d = self.stage3d(torch.cat((hx1to3d, hx2to3d, hx3to3d, hx6to3d, hx5dto3d, hx4dto3d), 1))
        # ##
        hx3d = self.stage3dfunc(hx1, hx2, hx3, hx6,
                                hx4d)
        ...
        # hx2d = self.stage2d(torch.cat((hx1to2d, hx2to2d, hx6to2d, hx5dto2d, hx4dto2d, hx3dto2d), 1))
        # ##
        hx2d = self.stage2dfunc(hx1, hx2, hx6,
                                hx4d, hx3d)
        ...
        # hx1d = self.stage1d(torch.cat((hx1to1d, hx6to1d, hx5dto1d, hx4dto1d, hx3dto1d, hx2dto1d), 1))
        # ##
        hx1d = self.stage1dfunc(hx1, hx6, hx4d,
                                hx3d, hx2d)
        # side output
        d1 = self.side1(hx1d)
        d1 = _upsample_like(d1, x)
        ...
        return [F.sigmoid(d0), F.sigmoid(d1), F.sigmoid(d2), F.sigmoid(d3), F.sigmoid(d4), F.sigmoid(d6)], [hx1d, hx2d,
                                                                                                            hx3d, hx4d,
                                                                                                            hx6]
