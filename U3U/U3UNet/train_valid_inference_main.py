...
from models import *
from parallel_u3u_net import ParallelU3UNet
...
def train(net, optimizer, train_dataloaders, train_datasets, valid_dataloaders, valid_datasets, hypar,train_dataloaders_val, train_datasets_val): #model_path, model_save_fre, max_ite=1000000):
    ...
    net.train() # set net to train condition
        ...
        for i, data in enumerate(gos_dataloader):
            ...
            if ite_num % model_save_fre == 0:  # validate every 2000 iterations
                ...
                if(tmp_out):
                    ...
                    #             "_mae_" + meanM + \
                    #             "_time_" + str(np.round(np.mean(np.array(tmp_time))/batch_size_valid,6))+".pth"
                    model_name = "/gpu_itr_"+str(ite_num)+".pth"
                    torch.save(net.state_dict(), model_path + model_name)
...
if __name__ == "__main__":
    ...
    dataset_tr = {"name": "Fire-TR",
                 "im_dir": "../Fire/Fire_TR/im",
                 "gt_dir": "../Fire/Fire_TR/gt",
                 "im_ext": ".jpg",
                 "gt_ext": ".png",
                 "cache_dir":"../Fire-Cache/Fire-TR"}

    dataset_vd = {"name": "Fire-VD",
                 "im_dir": "../Fire/Fire_VD/im",
                 "gt_dir": "../Fire/Fire_VD/gt",
                 "im_ext": ".jpg",
                 "gt_ext": ".png",
                 "cache_dir":"../Fire-Cache/Fire-VD"}
    ...
    hypar["interm_sup"] = False ## in-dicate if activate intermediate feature supervision // default: False
    # hypar["interm_sup"] = True

    if hypar["mode"] == "train":
        hypar["valid_out_dir"] = "" ## for "train" model leave it as "", for "valid"("inference") mode: set it according to your local directory
        hypar["model_path"] ="../saved_models/IS-Net-Fire" ## model weights saving (or restoring) path
        ...
        hypar["gt_encoder_model"] = "" # model name
    else: ## configure the segmentation output path and the to-be-used model weights path
        hypar["valid_out_dir"] = "../fire-results/"##"../DIS5K-Results-test" ## output inferenced segmentation maps into this fold
    ...
    hypar["model"] = U3U515BiasLayer()
    # hypar["model"] = U3U414()
    # os.environ['CUDA_VISIBLE_DEVICES'] = '1,0'
    # device = torch.device("cuda:1")
    #
    # if torch.cuda.device_count() > 1:
    #     hypar["model"] = nn.DataParallel(hypar["model"], device_ids=[0, 1])
    # hypar["model"].to(device)
