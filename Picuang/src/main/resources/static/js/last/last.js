                                ...
                                .then(function (dayRes) {
                                    $.each(dayRes.data, function (key, data) {
                                        ...
                                        $.each(type, function (key, t) {
                                            ...
                                            axios.get('/api/getLastPic?year=' + year + '&month=' + month + '&day=' + day + '&type=' + t)
                                                .then(function (listRes) {
                                                    console.log(listRes)
                                                    if(!(listRes.data == "" || listRes.data == null))
                                                    {
                                                        $("#" + year + "-" + month + "-" + day + "-" + t).append("<a href='" + listRes.data.path + "' target='_blank'><img class='lazyload img-thumbnail' src='background/load.gif' data-src='" + listRes.data.path + "' style='width: auto; height: auto; max-width: 128px; max-height: 128px; margin: 8px 4px'></a>");
                                                        if (i == 0) {
                                                            $("#histories").prepend("" +
