...
def callback(data):
    ...
    # 接收到-1表示俯视摄像头已经接收到火焰信息，当前飞控任务结束
    if num == -1:
        # pygame.quit()
    ...
    # 刚起飞还未检测到时，进行旋转自检
    if num == 10:
        yaw_rate = -1 * scale_ratio * base_yaw_rate
...
def main():
    rospy.init_node('camera1_uav_control', anonymous=True)
    rospy.Subscriber("/airsim/camera_1/xy_frame", Int32, callback)
